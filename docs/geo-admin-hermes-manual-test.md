...
| ID | 标题 | 依赖场景 |
|----|------|----------|
| TC-NAV-01 | 未登录访问受保护页重定向 | A 或 B |
| TC-AUTH-01 | 登录成功进入总览 | A 或 B |
| TC-DASH-01 | 总览台展示与入口 | A 与 B 分别测 |
| TC-RUN-01 | 运行体检与结果展示 | A 或 B |
| TC-HIST-01 | 历史列表与未关问题列 | A（B 可能无列表） |
| TC-HIST-02 | 单次详情：分数 + 结构化问题 + Markdown | A |
| TC-ISS-01 | 问题中心列表与 runId | A |
| TC-ISS-02 | 问题详情与决策提交 | A + 已存在 issue |
| TC-HUB-01 | 决策 / 验证 / 标准占位页 | A 或 B |
| TC-AUTH-02 | 退出登录 | A 或 B |
| TC-API-01 | 未带 Cookie 调 API 返回 401 | A 或 B |
| TC-API-02 | 带 Cookie 调 run 返回 issues 数组 | A 或 B（B 不入库；需服务端能执行 Python） |
