...
| 顺序 | 动作 | 对应 Hermes / 说明 |
|------|------|---------------------|
| 1 | 未登录访问受保护页，断言重定向到 login | TC-NAV-01 |
| 2 | 打开 `/{locale}/admin/login`，填写密码，点「登录」 | TC-AUTH-01 |
| 3 | 断言进入总览，出现「总览台」标题 | TC-DASH-01 |
| 4a | **默认**：页头「运行体检」→ 点击「运行 GEO 体检」→ 最长等待 120s 至再次可点 | TC-RUN-01 |
| 4b | **SKIP**：仅打开运行页断言标题，再回「总览」 | 快速路径 |
| 5 | 断言页面含「运行成功」或「运行失败」（仅非 SKIP） | TC-RUN-01 |
| 6 | 若存在「查看本次记录」则进入报告详情并断言「分数摘要」「结构化问题」 | TC-HIST-02（分支） |
| 7 | 页头「历史」→ 历史列表；若表格有链接则点第一条进详情并断言「分数摘要」 | TC-HIST-01 / 02 |
| 8 | 页头「问题」→ 问题中心；若表内有 issue 链接则点第一条进「问题详情」 | TC-ISS-01 / 02 |
| 9 | 直接访问「决策 / 验证 / 标准」页并断言各页 H2 | TC-HUB-01 |
| 10 | 回到总览 | — |
| 11 | 点击「退出登录」，断言回到登录页 | TC-AUTH-02 |
| 12 | 退出后再次访问受保护页，断言仍重定向到 login | TC-AUTH-02 加强版 |

脚本**不包含**：纯 API 调用 401（TC-API-01，由集成测试覆盖）、决策表单提交（TC-ISS-02，需真实 DB/issue 数据）。
